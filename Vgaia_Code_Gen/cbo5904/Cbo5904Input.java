...
/**
 * Cbo5904Input Class's Description <br>
 * Input State process of CboTBond_Contract Class.
 * 
 * <p>
 ...
 */
public class Cbo5904Input extends Frw00AbstractBusinessState {
	...
	public void initState(String before) throws Frw10ClientException {
		Cbo5904Frame frame = (Cbo5904Frame) getBaseTemplateFrame();
		Cbo5904Panel clientArea = (Cbo5904Panel) getClientPanel();
		...
		if (Frw00HeaderData.WNDSTAT_RESULT.equals(before)) {
			...
			if (!clientArea.F2) {
				clientArea.cboPaymentMethod.setValue(CboTIntPaymentMethod.CASH
						.toString());
				clientArea.cboIntPaymentMethod
						.setValue(CboTIntPaymentMethod.BANK_TRANSFER.toString());
				clientArea.pnlPaymentAccoInfo.setVisible(true);
				clientArea.lblAvailQty.setText(BigDecimal.ZERO);
				clientArea.txtContractNo.reset();
				clientArea.txtContractDate.setValue(PRHelper.getPrData()
						.getTransactionDate());
			}
	...
	public void setSuccessResultData(Frw00ResultData recvData)
		...
		Cbo5904ResultData resultData = (Cbo5904ResultData) recvData;
		...
		// Bind data to panel
		clientArea.bindDataToPanel();

		// Set data to info panel if update or delete mode
		if (!ItemValidator.isEntryMode(clientArea.cboActionMode)) {
			clientArea.bindPaymentAccInfo();
		}
	}
}
